...
{
    public $categories, $product_search, $products, $quantityCount = 1, $inputCategories = [];
    protected $queryString = [
        'inputCategories' => ['except' => '', 'as' => 'category'],
    ];

    public function mount($categories, $productsearch)
    {
        $this->categories = $categories;
        $this->product_search = $productsearch;
    }
    ...
    public function render()
    {
        ...
        $temp = array();
        foreach ($this->product_search as $item) {
            $temp[] = $item->name;
        }
        $this->products = Product::whereIn('name', $temp)
            ->when($this->inputCategories, function ($q) {
                $q->whereIn('category_id', $this->inputCategories);
            })
            ->get();
        return view('livewire.frontend.search.collection');
    }
}
